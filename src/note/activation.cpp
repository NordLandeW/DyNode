...
void NoteActivationManager::set_range(double curTime, double curSpeed) {
    ...
    timeRange = {currentTime,
                 currentTime + (ACTIVATION_AHEAD_PIXELS + BASE_RES_H -
...
void NoteActivationManager::recalculate() {
    ...
    for (const auto& note_ptr : poolMan.holdArray) {
        ...
        if (note.lastTime < timeRange.second - timeRange.first)
            break;
        if (note.time <= timeRange.first &&
            note.time + note.lastTime > timeRange.second) {
