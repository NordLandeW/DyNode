...
if((drawVisible || nodeAlpha > EPS || infoAlpha > EPS)) {
    ...
    // Draw Information
    if(infoAlpha > EPS) {
    	var _dx = 20, _dy = (noteType == 2? dFromBottom:23) * _inv,
    		_dyu = (noteType == 2? dFromBottom:25) * _inv;
	    scribble(string_format(position, 1, 2))
	    	.starting_format("mDynamix", c_aqua)
	    ...
	    scribble(_time)
	    	.starting_format("mDynamix", scribble_rgb_to_bgr(0xb2fab4))
	    	.blend(c_white, infoAlpha)
	    	.align(fa_right, fa_middle)
			.scale(_scale, _scale)
	    	.draw(x - _dx, y - _dyu);
	    ...
	    if(is_struct(lastAttachBar) && lastAttachBar.bar != undefined) {
	    	...
	    	scribble(_bar)
		    	.starting_format("mDynamix", scribble_rgb_to_bgr(0xFFE082))
		    	.blend(c_white, infoAlpha)
		    	.align(fa_left, fa_middle)
				.scale(_scale, _scale)
		    	.draw(x + _dx, y - _dyu);
